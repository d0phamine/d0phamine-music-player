...
export class PlayerStore {
    ...
    public setSelectedTrackInCurrentPlaylist(track?: ITrack, direction?: "next" | "previous") {
        if (direction) {
            const currentIndexOfSelectedTrack = this.playerData.currentPlaylist.findIndex((track) => track.selected);

            if (currentIndexOfSelectedTrack !== -1) {
                this.playerData.currentPlaylist[currentIndexOfSelectedTrack].selected = false;

                let newIndex;

                if (direction === "next") {
                    newIndex = (currentIndexOfSelectedTrack + 1) % this.playerData.currentPlaylist.length;
                } else if (direction === "previous") {
                    newIndex = (currentIndexOfSelectedTrack - 1 + this.playerData.currentPlaylist.length) % this.playerData.currentPlaylist.length;
                }

                if (newIndex !== undefined && newIndex >= 0 && newIndex < this.playerData.currentPlaylist.length) {
                    this.playerData.currentPlaylist[newIndex].selected = true;
                    this.playerData.selectedTrack = this.playerData.currentPlaylist[newIndex];
                }
            }
        } else {
    ...
    public shufflePlaylist() {
        ...
        for (let i = this.playerData.currentPlaylist.length - 1; i > 0; i--) {
            ...
            [this.playerData.currentPlaylist[i], this.playerData.currentPlaylist[j]] = [this.playerData.currentPlaylist[j], this.playerData.currentPlaylist[i]];
        }
    ...
    public initMediaSession(howlerRef: React.RefObject<ReactHowler>) {
        if ("mediaSession" in navigator) {
            const updateMetadata = () => {
                ...
                if (selectedTrack) {
                    const artwork = selectedTrack.cover ? [{ src: selectedTrack.cover }] : [];
            ...
            const handlers = {
                ...
                previoustrack: () => this.setSelectedTrackInCurrentPlaylist(undefined, "previous"),
                nexttrack: () => this.setSelectedTrackInCurrentPlaylist(undefined, "next"),
                seekbackward: (details: MediaSessionSeekOffset) => {
                    const seekOffset = details.seekOffset || 10;
                    const newTime = Math.max((this.playerData.currentSeekOfPlay || 0) - seekOffset, 0);
                    this.setCurrentSeekOfPlay(newTime);
                    this.updateHowlerSeek(newTime, howlerRef);
                },
                seekforward: (details: MediaSessionSeekOffset) => {
                    const seekOffset = details.seekOffset || 10;
                    const newTime = Math.min((this.playerData.currentSeekOfPlay || 0) + seekOffset, this.playerData.selectedTrack?.duration || 0);
                    this.setCurrentSeekOfPlay(newTime);
                    this.updateHowlerSeek(newTime, howlerRef);
                },
                seekto: (details: MediaSessionSeekToActionDetails) => {
                    if (typeof details.seekTime === "number" && howlerRef.current) {
                        const duration = this.playerData.selectedTrack?.duration || 0;
    ...
    private updateHowlerSeek(time: number, howlerRef: React.RefObject<ReactHowler>) {
